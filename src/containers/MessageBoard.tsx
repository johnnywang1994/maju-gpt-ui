...
const validImageModels = ['dall-e-2', 'dall-e-3', 'gpt-image-1', 'gemini-2.5-flash-image-preview'];
  ...
  const handleMessages = (newMessage: SendMessage) => {
    ...
    let sendMessages = messages.map(({ role, content }) => ({
      ...
      if (typeof content === "string") {
        return !content.startsWith('https://') || !content.startsWith('data:image/');
      }
      return true;
    }); // remove image generated by GPT
  ...
  const handleSendRequest = async (newMessage: SendMessage) => {
    ...
    try {
      ...
      if (
        ...
        !validImageModels.includes(options.model)
      ) {
        message.error(`Please use model ${validImageModels.map(model => `"${model}"`).join(", ")} to generate image.`);
        return;
      }
